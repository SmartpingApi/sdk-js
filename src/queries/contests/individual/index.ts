...
export default class IndividualContestQueries {
	...
	getFederalCriteriumRank(divisionId: number, options?: QueryOptions) {
		return GetFederalCriteriumRankForDivision.create(this.#context).withOptions(options).run(divisionId);
	}

	getGames(contestId: number, divisionId: number, groupId?: number, options?: QueryOptions) {
		return GetIndividualContestGames.create(this.#context).withOptions(options).run(contestId, divisionId, groupId);
	}

	getGroups(contestId: number, divisionId: number, options?: QueryOptions) {
		return GetIndividualContestGroups.create(this.#context).withOptions(options).run(contestId, divisionId);
	}

	getRank(contestId: number, divisionId: number, groupId?: number, options?: QueryOptions) {
		return GetIndividualContestRank.create(this.#context).withOptions(options).run(contestId, divisionId, groupId);
	}
}
