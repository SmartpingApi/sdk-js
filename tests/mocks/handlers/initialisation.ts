...
export default http.get(endpoint(ApiEndpoints.XML_INITIALISATION), ({ request }) => {
	...
	if (invalidParameters) {
		return HttpResponse.xml(getMockResponse('initialisation_erreur', { message: 'Paramètre manquant' }), {
			status: 401,
	...
	if (url.searchParams.get('id') !== env.SMARTPING_APP_ID) {
		return HttpResponse.xml(getMockResponse('initialisation_erreur', { message: 'Compte incorrect' }), {
			status: 401,
			headers: errorHeaders,
		});
	}
	...
	return HttpResponse.xml(
		getMockResponse('initialisation', {}),
